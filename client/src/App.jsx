...
function App() {
  ...
  const fetchData = async () => {
    ...
    try {
      const [p, t, n, m, qnu] = await Promise.all([
        axios.get(`http://localhost:5000/api/posts?userId=${user.id}`),
        axios.get(`http://localhost:5000/api/tasks/${user.id}`),
        axios.get(`http://localhost:5000/api/notifications/${user.id}`),
        axios.get(`http://localhost:5000/api/market`),
        axios.get(`http://localhost:5000/api/qnu/schedules/${user.id}`)
      ]);
      setPosts(p.data || []);
      setTasks(t.data || []);
  ...
  const fetchFriends = async () => { if (!user) return; try { const res = await axios.get(`http://localhost:5000/api/friends/${user.id}`); setFriends(res.data.friends || []); setPendingRequests(res.data.pendingRequests || []); setSentRequests(res.data.sentRequests || []); } catch (err) { } };
  const fetchProfileInfo = async () => { if (!user) return; try { const res = await axios.get(`http://localhost:5000/api/users/${user.id}/profile`); setProfileForm(res.data.profile); setUser(prev => { if (!prev || !localStorage.getItem('universe_user')) return null; const up = { ...prev, ...res.data.profile }; localStorage.setItem('universe_user', JSON.stringify(up)); return up; }); } catch (e) { } };

  const handleImageUploadLocal = async (file) => {
    if (!file) return null; const formData = new FormData(); formData.append('image', file);
    try { const res = await axios.post('http://localhost:5000/api/upload', formData); return res.data.url; } catch (err) { return null; }
  }
  ...
  const handleDeletePost = async (postId) => { if (window.confirm("Xóa bài viết này vĩnh viễn?")) { await axios.delete(`http://localhost:5000/api/posts/${postId}`); fetchData(); if (activeTab === 'user-profile') openUserProfile(viewingProfile.id); } };
  const handleEditPostSubmit = async (postId) => { await axios.put(`http://localhost:5000/api/posts/${postId}`, { content: editPostContent }); setEditingPostId(null); fetchData(); if (activeTab === 'user-profile') openUserProfile(viewingProfile.id); };
  const handleHidePost = async (postId) => { await axios.post(`http://localhost:5000/api/posts/${postId}/hide`, { userId: user.id }); fetchData(); if (activeTab === 'user-profile') openUserProfile(viewingProfile.id); };
  const handleReactPost = async (e, postId, type) => { e.stopPropagation(); await axios.post(`http://localhost:5000/api/posts/${postId}/react`, { userId: user.id, type }); setActiveReactionPopup(null); fetchData(); if (activeTab === 'user-profile') openUserProfile(viewingProfile.id); };
  const handleLikeComment = async (commentId) => { await axios.post(`http://localhost:5000/api/comments/${commentId}/like`, { userId: user.id }); fetchData(); if (activeTab === 'user-profile') openUserProfile(viewingProfile.id); };
  const handleCommentSubmit = async (e, postId) => { e.preventDefault(); const content = newCommentTexts[postId]; if (!content?.trim()) return; await axios.post(`http://localhost:5000/api/posts/${postId}/comment`, { userId: user.id, content, parentId: replyingTo[postId] || null }); setNewCommentTexts({ ...newCommentTexts, [postId]: "" }); setReplyingTo({ ...replyingTo, [postId]: null }); fetchData(); if (activeTab === 'user-profile') openUserProfile(viewingProfile.id); };
  const handleShareSubmit = async () => { if (!shareModalData) return; const postUrl = `http://localhost:3000/post/${shareModalData.id}`; const finalContent = `${shareText}\n\n--- ️ Chia sẻ bài viết của ${shareModalData.user.fullName || shareModalData.user.username} ---\n${postUrl}\n\n"${shareModalData.content}"`; await axios.post('http://localhost:5000/api/posts', { content: finalContent, userId: user.id }); setShareModalData(null); setShareText(""); fetchData(); showAlert("Đã chia sẻ lên Bảng tin!"); };
  ...
  const handleAddTask = async (e) => { e.preventDefault(); if (!newTaskDate) return showAlert("Chọn hạn nộp!"); await axios.post('http://localhost:5000/api/tasks', { title: newTaskTitle, dueDate: newTaskDate, color: `border-indigo-500`, userId: user.id }); setNewTaskTitle(""); setNewTaskDate(""); fetchData(); };
  const handleLMSScan = async () => { const res = await axios.post('http://localhost:5000/api/tasks/scan-lms', { userId: user.id }); fetchData(); showAlert(res.data.message); };
  ...
  const handleQnuSync = async (e) => {
    ...
    setTimeout(() => {
      ...
      showAlert("Đã đồng bộ 7 môn học!", "info");
    }, 5000);
  };

  const handleDraftToAction = async (id) => { setIsDraftingId(id); try { await axios.post(`http://localhost:5000/api/tasks/${id}/draft`); fetchData(); } catch (err) { } setIsDraftingId(null); };
  const handleDeleteTask = async (id) => { await axios.delete(`http://localhost:5000/api/tasks/${id}`); fetchData(); };
  ...
  const handleBuyItem = async (item) => { if (coins < item.reward) return showAlert("Không đủ UC!"); const res = await axios.post('http://localhost:5000/api/market/buy', { itemId: item.id, buyerId: user.id }); if (res.data.success) { setCoins(res.data.newCoinBalance); fetchData(); showAlert("Thanh toán thành công!"); } else { showAlert(res.data.message); } };
    ...
    navigator.geolocation.getCurrentPosition(async (pos) => {
      ...
      try {
        if (type === 'ERRAND') {
          ...
          await axios.post('http://localhost:5000/api/posts', { content, userId: user.id });
          fetchData();
  ...
  const handleScanImage = async () => { if (!selectedImage) return showAlert("Chọn ảnh bài giảng!"); setIsOcrLoading(true); setOcrOutput("AI đang quét..."); const formData = new FormData(); formData.append('image', selectedImage); try { const res = await axios.post('http://localhost:5000/api/ai/ocr', formData, { headers: { 'Content-Type': 'multipart/form-data' } }); setOcrOutput(res.data.output); } catch (error) { setOcrOutput("Lỗi!"); } finally { setIsOcrLoading(false); } };

  const handleAuthSubmit = async (e) => { e.preventDefault(); setAuthError(''); try { const res = await axios.post(`http://localhost:5000${authMode === 'login' ? '/api/auth/login' : '/api/auth/register'}`, authForm); if (res.data.success) { setUser(res.data.user); localStorage.setItem('universe_user', JSON.stringify(res.data.user)); } else { setAuthError(res.data.message); } } catch (err) { } };
  ...
  const handleUpdateProfile = async (e) => {
    ...
    try {
      ...
      const res = await axios.put(`http://localhost:5000/api/users/${user.id}`, validData);
      if (res.data.success) {
  ...
  const openUserProfile = async (userId) => {
    ...
    try { const res = await axios.get(`http://localhost:5000/api/users/${userId}/profile`); setViewingProfile(res.data.profile); setViewingPosts(res.data.posts || []); } catch (error) { }
  };
  ...
  const handleAddFriend = async (friendId) => { await axios.post('http://localhost:5000/api/friends/request', { userId: user.id, friendId }); showAlert("Đã gửi lời mời!"); setSearchQuery(""); setSearchResults([]); fetchFriends(); };
  const handleAcceptFriend = async (reqId) => { await axios.put('http://localhost:5000/api/friends/accept', { reqId }); fetchFriends(); };
  const handleDeclineFriend = async (reqId) => { await axios.delete(`http://localhost:5000/api/friends/decline/${reqId}`); fetchFriends(); };
  ...
  const openChat = async (friend, refItem = null, refType = null) => {
    ...
    const res = await axios.get(`http://localhost:5000/api/messages/${user.id}/${friend.id}${params}`);
  ...
  const handleAcceptOffer = async (msg) => {
    ...
    try {
      if (chatRefType === 'DOCUMENT' && chatRefItem) {
        ...
        // Xác nhận phía mình
        await axios.post(`http://localhost:5000/api/documents/confirm-sale/${chatRefItem.id}`, {
        ...
        await axios.put(`http://localhost:5000/api/errands/complete/${chatRefItem.id}`, { requesterId: user.id });
      }
  ...
  const handleReactMessage = async (msgId, type) => { await axios.post(`http://localhost:5000/api/messages/${msgId}/react`, { userId: user.id, type }); const res = await axios.get(`http://localhost:5000/api/messages/${user.id}/${activeChat.id}`); setMessages(res.data || []); setActiveMsgReact(null); };
  const handleCheckIn = () => {
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      {filteredTasks.length === 0 ? <p className="text-center col-span-full py-4 text-gray-400 font-bold">Không tìm thấy bài tập nào.</p> : (
                        <>
                          {filteredTasks.filter(t => new Date(t.dueDate).setHours(0, 0, 0, 0) >= new Date().setHours(0, 0, 0, 0)).map(t => (
                            <div key={t.id} className={`p-5 rounded-2xl shadow-sm border border-l-4 border-l-indigo-500 flex flex-col justify-between ${panicMode ? 'bg-slate-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                              <div>
                                <div className="flex justify-between items-start mb-2"><h4 className={`font-bold text-[16px] leading-tight pr-4 ${panicMode ? 'text-white' : 'text-gray-800'}`}>{t.title}</h4><button onClick={() => handleDeleteTask(t.id)} className="text-slate-400 hover:text-red-500 transition"><Icons.Trash className="w-5 h-5 opacity-80" /></button></div>
              ...
              {transHistoryData.length === 0 ? (
                ...
                <div key={i} className={`flex items-center gap-2 p-2 rounded-xl mb-2 cursor-pointer border transition ${panicMode ? 'bg-slate-800 border-gray-700 hover:bg-slate-700' : 'bg-white border-gray-200 hover:bg-indigo-50'}`}
                  onClick={() => {
                    ...
                    axios.get(`http://localhost:5000/api/messages/${user.id}/${activeChat.id}?refType=${tx.type}&refId=${tx.item.id}`).then(r => setMessages(r.data || []));
                    setShowTransHistory(false);
                  }}
                >
...
const GoogleClassroomSyncButton = ({ userId, onSyncSuccess, showAlert }) => {
  ...
  const login = useGoogleLogin({
    ...
    onSuccess: async (codeResponse) => {
      setIsSyncing(true);
      try {
        const res = await axios.post('http://localhost:5000/api/tasks/sync-google', {
          userId,
  ...
  return (
    <>
      <button
        onClick={() => login()}
        disabled={isSyncing}
        className="bg-white flex items-center gap-2 text-green-700 px-4 py-2 rounded-lg font-bold text-sm shadow hover:bg-green-50 transition"
      >
        <img src="https://upload.wikimedia.org/wikipedia/commons/5/59/Google_Classroom_Logo.png" alt="Google Classroom" className="w-5 h-5" />
        {isSyncing ? "Đang đồng bộ..." : "Classroom"}
      </button>
