...
app.post('/api/tasks/sync-google', async (req, res) => {
  try {
    ...
    if (!process.env.GOOGLE_CLIENT_SECRET || process.env.GOOGLE_CLIENT_SECRET === 'PLACEHOLDER') {
      const mockTasks = [
        { title: '[Classroom] Bài tập PHP', dueDate: '2026-04-30', color: 'border-green-500', isLMS: true, userId: parseInt(userId) },
        { title: '[Classroom] Tuần 31: Biểu đồ lớp phân tích', dueDate: '2026-05-01', color: 'border-green-500', isLMS: true, userId: parseInt(userId) }
      ];
...
// Xác ��9nh học kỳ và nĒm học theo quy tắc của trường
const getHocKyInfo = () => {
...
const scrapeQNUSchedule = async (username, password) => {
  ...
  try {
    ...
    // === BƯ�aC 1: Đ�NG NHẬP ===
    console.log('[QNU] Đang truy cập trang �Ēng nhập...');
    ...
    // Chờ chuyỒn trang (rời khỏi /login)
    await page.waitForURL(url => !url.includes('/login'), { timeout: 20000 }).catch(async () => {
      ...
      if (page.url().includes('/login')) throw new Error('ĐĒng nhập thất bại - vui lòng kiỒm tra lại thông tin.');
    });
    console.log('[QNU] ĐĒng nhập thành công, URL:', page.url());

    // === BƯ�aC 2: VìO TRANG TH�SI KH�A BI�U ===
    await page.goto('https://daotao.qnu.edu.vn/student/schedules', { waitUntil: 'networkidle', timeout: 30000 });
    await page.waitForTimeout(3000);

    // === BƯ�aC 3: CH�RN TAB "TH�SI KH�A BI�U THEO L�aP H�RC PHẦN" (tab �ầu tiên - index 0) ===
    try {
      const tabs = page.locator('[role="tab"], .MuiTab-root');
      const tabCount = await tabs.count();
      console.log(`[QNU] Tìm thấy ${tabCount} tab`);
      if (tabCount > 0) {
        // Tab �ầu tiên là "Thời khóa biỒu theo l�:p học phần"
        await tabs.first().click();
        console.log('[QNU] Đã click tab Thời khóa biỒu theo l�:p học phần');
      }
    } catch(e) {
      console.log('[QNU] L�i click tab:', e.message);
    }
    await page.waitForTimeout(2500);

    // === BƯ�aC 4: CH�RN N�M H�RC Vì H�RC KỲ ===
    const { hocKy, namHoc } = getHocKyInfo();
    console.log(`[QNU] Đang chọn: NĒm học ${namHoc} - ${hocKy}`);
      ...
      if (txt.includes('-') && (txt.includes('202') || txt.includes('201'))) {
        if (!txt.includes(namHoc)) {
          ...
          if (await option.count() > 0) {
            ...
            console.log(`[QNU] Đã chọn nĒm học: ${namHoc}`);
    ...
    // === BƯ�aC 5: CLICK N�aT T�RM KIẾM / HI�N TH�` ===
    try {
      const searchBtn = page.locator('button:has-text("Tìm kiếm"), button:has-text("Hi�!n tại"), button:has-text("Xem")').first();
      if (await searchBtn.count() > 0) {
        await searchBtn.click();
        console.log('[QNU] Đã click nút tìm kiếm');
      }
    } catch(e) { /* không bắt bu�"c */ }
    ...
    // === BƯ�aC 6: SCRAPE BẢNG DỮ LI� U ===
    // Cấu trúc bảng từ screenshot:
    // [0]STT [1]Mã l�:p/Tên [2]Giáo viên [3]STC [4]Thứ [5]Tiết [6]Phòng học [7]Từ ngày-�ến ngày [8]Phòng/��9a �iỒm [9]Đ�9a ch�0
    const rawSchedules = await page.evaluate(() => {
      ...
      for (const row of rows) {
        ...
        if (cols.length >= 8) {
          // Dòng �ầy �ủ có Mã l�:p/Tên �x c�"t 1
          ...
          if (monInfo && !monInfo.toLowerCase().includes('tên học phần') && !monInfo.toLowerCase().includes('mã l�:p')) {
            lastMon = monInfo;
          ...
          // Dòng phụ (không có tên môn, ch�0 có thứ/tiết/phòng/ngày)
    ...
    console.log(`[QNU] Cào �ược ${rawSchedules.length} dòng thô`);

    if (rawSchedules.length === 0) throw new Error('Không tìm thấy dữ li�!u TKB. Hãy kiỒm tra lại tài khoản và học kỳ.');

    // === BƯ�aC 7: XỬ LÝ DỮ LI� U ===
    ...
    const MAP_THU = { '2':'Thứ Hai','3':'Thứ Ba','4':'Thứ Tư','5':'Thứ NĒm','6':'Thứ Sáu','7':'Thứ Bảy','CN':'Chủ Nhật' };
    ...
    for (const item of rawSchedules) {
      ...
      // Xử lý ngày: "19/01/2026->11/05/2026" hoặc "19/01/2026 - 11/05/2026"
    ...
    console.log(`[QNU] Xử lý xong: ${finalSchedules.length} l�9ch học - NĒm học ${namHoc} ${hocKy}`);
    return finalSchedules;

  } catch (error) {
    console.error('[QNU Scraper] L�i:', error.message);
    if (error.message.includes('Tài khoản') || error.message.includes('Mật khẩu') || error.message.includes('�Ēng nhập')) throw error;
    throw new Error('Không thỒ ��ng b�" TKB từ QNU: ' + error.message);
  } finally {
...
app.post('/api/qnu/sync', async (req, res) => {
  try {
    ...
    if (!username || !password) return res.status(400).json({ success: false, message: 'Thiếu thông tin �Ēng nhập' });

    // Lưu credentials �Ồ cron job tự ��"ng sync hàng ngày
    ...
    res.json({ success: true, message: `Đã ��ng b�" ${schedules.length} l�9ch học từ QNU!`, data: schedules });
  } catch (err) {
    console.error('[QNU Sync API]', err.message);
    res.status(400).json({ success: false, message: err.message || 'Có l�i xảy ra trong quá trình ��ng b�".' });
  }
});

// CRON JOB TỰ Đ��NG Đ�NG B�� TKB L�aC 00:00 HìNG NGìY
cron.schedule('0 0 * * *', async () => {
  console.log('[CRON 00:00] Bắt �ầu tự ��"ng ��ng b�" TKB QNU cho tất cả user...');
  try {
    ...
    for (const u of users) {
      try {
        ...
        await sendNotification(u.id, 'H�RC TẬP', `�x️ Thời khóa biỒu của bạn �ã �ược ��ng b�" tự ��"ng! Có ${schedules.length} l�9ch học.`);
        console.log(`[CRON] Sync OK cho user ${u.fullName} (${u.id}): ${schedules.length} l�9ch`);
      } catch (e) {
        console.error(`[CRON] Sync FAIL cho user ${u.id}:`, e.message);
      }
    }
    console.log('[CRON 00:00] Hoàn thành ��ng b�" TKB.');
  } catch (err) {
    console.error('[CRON 00:00] L�i:', err.message);
  }
...
app.post('/api/auth/register', async (req, res) => { try { const { username, password, fullName, major } = req.body; const existing = await prisma.user.findUnique({ where: { username } }); if (existing) return res.json({ success: false, message: "Username �ã t�n tại!" }); const newUser = await prisma.user.create({ data: { username, password, fullName: fullName || username, major: major || '', coins: 250 } }); res.json({ success: true, user: newUser }); } catch (err) {} });
app.post('/api/auth/login', async (req, res) => { try { const { username, password } = req.body; if (username === 'admin' && password === '1') return res.json({ success: true, user: { id: 999999, username: 'admin', fullName: 'Quản tr�9 viên', role: 'ADMIN' } }); const user = await prisma.user.findFirst({ where: { username, password } }); if (!user) return res.json({ success: false, message: "Sai thông tin!" }); res.json({ success: true, user }); } catch (err) {} });
...
app.post('/api/admin/broadcast', async (req, res) => { const { content } = req.body; const users = await prisma.user.findMany({ select: { id: true } }); const notifs = users.map(u => ({ userId: u.id, type: '�x� TH�NG BÁO H�  THỐNG', content, isRead: false })); await prisma.notification.createMany({ data: notifs }); io.emit('new_notification', { type: '�x� H�  THỐNG', content, isRead: false }); res.json({ success: true }); });

const PORT = 5000;
server.listen(PORT, () => console.log(`�xa� Server �ang chạy: http://localhost:${PORT}`));
